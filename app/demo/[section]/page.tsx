...
export default async function DemoSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "strategist") return <StrategistClient studentName={DEMO_USER.name} initials={DEMO_USER.initials} grade={DEMO_USER.grade} contextRows={[{ k: "GPA", v: "3.80" }, { k: "Target", v: "Elite-tier · demo" }, { k: "Probability", v: "41% illustrative" }, { k: "Country", v: "Bangladesh" }]} gapRows={GAPS} benchmarkNote="Demo profile only. These values are illustrative; the signed-in Strategist derives references from the Polaris university dataset." eyebrow="Strategist · grounded · Elite-tier target" demo />;
