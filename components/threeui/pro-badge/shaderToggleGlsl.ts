// Adapted from supplied ThreeUI shader: omit thumb for the read-only PRO badge.
...
void main() {
  ...
  /* Light mode: warm pearl stock with copper filaments and restrained sparks. */
  vec3 pearl = mix(vec3(0.77, 0.52, 0.31), vec3(0.98, 0.88, 0.72), plasma);
  pearl += threads * threads * vec3(0.12, 0.09, 0.05);
  pearl += caustic * vec3(0.10, 0.08, 0.05) + streak * vec3(0.07, 0.05, 0.03);
  pearl += sparks * 0.16;
  vec3 lightBadge = mix(vec3(0.34, 0.22, 0.15), pearl, inside);
  lightBadge += vec3(0.36, 0.23, 0.12) * stroke(d, 0.025);
  color = mix(lightBadge, color, dark);

  /* PRO badge: the authored trough and sparks remain, without a thumb. */
